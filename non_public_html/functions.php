...
function verify_server() {
	...
	} elseif (HTTP_HOST === DOMAIN . '.' . REMOTE_TLD) {
...
function error_handler_php($number, $message, $file, $line, $vars) {
  return 'There was an error - please refresh your browser';
...
function get_long_url_and_go($short, $DBH) {
	...
	if ($result = $sth->fetch(PDO::FETCH_NUM)) {
		...
		if (is_long_valid($result[0]) || (stristr($result[0], 'http://www.avon.uk.com/PRSuite/eBrochure.page?'))) {
